"""Shared pytest setup for the solution1 tests.

The exercise modules import each other by flat module name (for example
``from greeting_activity import create_greeting``) so that ``start_worker.py``
can run as a plain script. Putting this directory on ``sys.path`` lets the
tests import them the same way regardless of where pytest is invoked from.
"""
